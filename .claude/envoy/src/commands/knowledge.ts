 ...
 *   envoy knowledge search-aggregated <query> [--force-aggregate]
...
/**
 * Search-aggregated command - semantic search with AI aggregation for large result sets
 */
class SearchAggregatedCommand extends BaseCommand {
  readonly name = "search-aggregated";
  readonly description = "Semantic search with AI aggregation (uses sub-agent for large results)";
  ...
  async execute(args: Record<string, unknown>): Promise<CommandResult> {
    ...
    try {
      ...
      const results = await service.search(query, 50, false);
      ...
      return this.error("search_error", e instanceof Error ? e.message : String(e));
    }
  }
  ...
  private formatAggregatorInput(
    ...
    const input: AggregatorInput = {
      ...
      minimized_results: minimizedResults,
    };
...
export const COMMANDS = {
  ...
  "search-aggregated": SearchAggregatedCommand,
