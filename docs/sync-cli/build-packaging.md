...
CLI distributed via npm, bundled with esbuild into single executable. Bundling simplifies installation (no node_modules resolution at runtime), enables npx all-hands workflow. Dotfile restoration handles npm's hardcoded exclusions.
...
**npx installation**: User runs `npx all-hands init .` - npm downloads package, runs bundled CLI, CLI copies distributed files including restoring dotfiles. No prior installation needed.
...
**Version pinning**: Consumer can install specific version `npx all-hands@1.0.x init .` Package version in package.json tracks distributed file versions. Update to newer CLI version brings newer configs.
