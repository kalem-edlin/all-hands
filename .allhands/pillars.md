...
Prompt files are atomic units of work executed in tracked loops. Planned prompts deliver scoped tasks from the planner; emergent prompts are created by the hypothesis planner as non-overlapping hypotheses for executors to implement. Both follow the same execution primitive — the distinction is origin (planned vs hypothesis-generated), not execution path. Alignment documents track summaries and decisions across iterations.
